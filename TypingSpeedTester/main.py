...
class Word_Generator:
    ...
    def next_word(self):
        ...
        canvas.coords(next_word_label, new_x_location, 200)
...
# Word Prompt
active_word_label = canvas.create_text(380, 200, anchor=W, text="")
canvas.itemconfig(active_word_label, font=H2_FONT, fill=ACTIVE_TEXT_COLOUR)
previous_word_label = canvas.create_text(360, 200, anchor=E, text="")
canvas.itemconfig(previous_word_label, font=H2_FONT, fill=TEXT_COLOUR)
next_word_label = canvas.create_text(380, 200, anchor=W, text="")
canvas.itemconfig(next_word_label, font=H2_FONT, fill=TEXT_ACCENT_COLOUR)
starting_word_label = canvas.create_text(400, 200, anchor=CENTER, text="Ready? Hit 'Start' To Begin!")
...
# Timer
timer_label = canvas.create_text(150, 477, anchor=CENTER, text="Time Remaining")
canvas.itemconfig(timer_label, font=H3_FONT, fill=TEXT_COLOUR)
timer = canvas.create_text(150, 580, anchor=CENTER, text="--")
canvas.itemconfig(timer, font=H1_FONT, fill=TEXT_COLOUR)

# Word Score
word_score_label = canvas.create_text(400, 477, anchor=CENTER, text="Words Per Minute")
canvas.itemconfig(word_score_label, font=H3_FONT, fill=TEXT_COLOUR)
word_score = canvas.create_text(400, 580, anchor=CENTER, text="--")
canvas.itemconfig(word_score, font=H1_FONT, fill=TEXT_COLOUR)

# Accuracy
accuracy_label = canvas.create_text(650, 477, anchor=CENTER, text="Accuracy (%)")
canvas.itemconfig(accuracy_label, font=H3_FONT, fill=TEXT_COLOUR)
accuracy = canvas.create_text(650, 580, anchor=CENTER, text="--")
canvas.itemconfig(accuracy, font=H1_FONT, fill=TEXT_COLOUR)

# Start Button
start_button = Button(text="START", command=word.start_button)
...
canvas.create_window(100, 679, window=start_button)

# Reset Button
reset_button = Button(text="RESET", command=word.reset_button)
...
canvas.create_window(100, 754, window=reset_button)

# Instructions
instructions_label = canvas.create_text(475, 670, anchor=CENTER, text="~~ Instructions ~~")
canvas.itemconfig(instructions_label, font=H3_FONT, fill=TEXT_ACCENT_COLOUR)
instructions_line_1 = canvas.create_text(210, 695, anchor=W, text="~ Place cursor in centre box and hit 'Start' button")
canvas.itemconfig(instructions_line_1, font=H4_FONT, fill=TEXT_ACCENT_COLOUR)
instructions_line_2 = canvas.create_text(210, 715, anchor=W, text="~ Timer will run for 60 seconds")
canvas.itemconfig(instructions_line_2, font=H4_FONT, fill=TEXT_ACCENT_COLOUR)
instructions_line_3 = canvas.create_text(210, 735, anchor=W, text="~ Hitting 'Space' moves to next word")
canvas.itemconfig(instructions_line_3, font=H4_FONT, fill=TEXT_ACCENT_COLOUR)
instructions_line_4 = canvas.create_text(210, 755, anchor=W, text="~ Hit 'Reset' button to stop and go again")
